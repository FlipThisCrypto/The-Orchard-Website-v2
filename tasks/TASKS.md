# TASKS — The Orchard v2

> Auto-generated from `tasks/tasks.json` by `scripts/generate.mjs`. **Do not edit by hand** —
> edit the JSON and regenerate. Live view: `dashboard/index.html`.

**Overall: 48% complete** — 11/23 tasks · updated 2026-06-17

## By advisor

| Seat | Owner | Done | Total | % |
|---|---|---|---|---|
| Lead Dev & Integrator | Claude Code | 8 | 11 | 73% |
| Product & UX Strategy | ChatGPT | 1 | 5 | 20% |
| Research & Modeling | Gemini | 1 | 4 | 25% |
| Growth & Markets | Grok | 1 | 3 | 33% |

## Phase 0 — Foundation — 100%

The coordination system: manual, roles, templates, task board, dashboard, brand & architecture grounding.

| ID | Task | Owner | Status | Pri | Deliverable |
|---|---|---|---|---|---|
| ORCH-001 | Repo structure + AGENTS.md operating manual | Claude Code | ✅ done | P0 | `AGENTS.md` |
| ORCH-002 | Task system (tasks.json + TASKS.md generator) | Claude Code | ✅ done | P0 | `tasks/tasks.json` |
| ORCH-003 | Live progress dashboard | Claude Code | ✅ done | P0 | `dashboard/index.html` |
| ORCH-004 | MISSION.md grounding pack | Claude Code | ✅ done | P0 | `MISSION.md` |
| ORCH-005 | Brand & voice reference | Claude Code | ✅ done | P1 | `docs/brand/brand-and-voice.md` |
| ORCH-006 | Advisor guides + templates | Claude Code | ✅ done | P0 | `governance/roles/` |
| ORCH-007 | Globe/Atlas architecture brief (ADR) | Claude Code | ✅ done | P1 | `docs/architecture/0001-atlas-globe.md` |
| ORCH-008 | Decision log seeded (ADR 0001, 0002) | Claude Code | ✅ done | P1 | `governance/decisions/` |

## Phase 1 — Direction — 21%

Advisors answer the big strategic questions in their lanes; Lead synthesizes into locked decisions (ADRs).

| ID | Task | Owner | Status | Pri | Deliverable |
|---|---|---|---|---|---|
| ORCH-010 | 'The Orchard in 2036' positioning + category | ChatGPT | ✅ done | P0 | `docs/product/positioning-2036.md` |
| ORCH-011 | Persona ranking + the single 60-second action | ChatGPT | ⏳ assigned | P0 | `docs/product/personas-and-primary-action.md` |
| ORCH-012 | Information architecture / sitemap + progressive disclosure | ChatGPT | ▢ ready | P1 | `docs/product/information-architecture.md` |
| ORCH-013 | Fruit -> data-class legend | ChatGPT | ▢ ready | P1 | `docs/product/fruit-data-legend.md` |
| ORCH-014 | Node-state & gamification model | ChatGPT | ▢ ready | P2 | `docs/product/node-states-gamification.md` |
| ORCH-020 | DePIN competitor teardown | Gemini | ✅ done | P1 | `docs/research/competitors.md` |
| ORCH-021 | Data-buyer & use-case research | Gemini | ⏳ assigned | P1 | `docs/research/data-buyers-and-use-cases.md` |
| ORCH-022 | Financial model v0 | Gemini | ▢ ready | P2 | `docs/research/financial-model-v0.md` |
| ORCH-023 | Nonprofit / compliance research | Gemini | ▢ ready | P2 | `docs/research/nonprofit-compliance.md` |
| ORCH-030 | Positioning & messaging pillars | Grok | ✅ done | P1 | `docs/growth/positioning-and-messaging.md` |
| ORCH-031 | Community & growth plan | Grok | ⏳ assigned | P2 | `docs/growth/community-and-growth-plan.md` |
| ORCH-032 | Real-time DePIN market pulse | Grok | ▢ ready | P2 | `docs/growth/depin-market-pulse.md` |
| ORCH-041 | Performance budget + device targets | Claude Code | ▢ ready | P2 | `docs/architecture/performance-budget.md` |
| ORCH-042 | Atlas data & privacy contract mapping | Claude Code | ▢ ready | P2 | `docs/architecture/atlas-data-privacy-contract.md` |

## Phase 2 — Design — 0%

Wireframes, design tokens, globe proof-of-concept.

| ID | Task | Owner | Status | Pri | Deliverable |
|---|---|---|---|---|---|
| ORCH-040 | Globe proof-of-concept (3 real Trees) | Claude Code | 💤 backlog | P1 | `prototypes/globe-poc/` |

